...
## 讀 TensorTS 清晨原始長表
tensor_night <- readr::read_csv("tensor_night.csv")


## 統一命名（請按你的實際欄位名調整以下 rename）
tensor_night_std <- tensor_night %>%
...
tensor_night_std <- tensor_night_std %>%
...
tensor_night_canonical <- tensor_night_std %>%
  select(
...
readr::write_csv(tensor_night_canonical, "tensor_night_canonical.csv")
