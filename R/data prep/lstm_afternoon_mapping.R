...
lstm_afternoon <- readr::read_csv("lstm_afternoon.csv")  

library(tidyverse)  # 會載入 dplyr（含 %>%）、readr 等
library(janitor)    # 提供 clean_names()

# 統一欄位命名（依你的實際欄位調整）
lstm_afternoon_std <- lstm_afternoon %>%
  clean_names() %>%
  rename(
    lstm_o   = from,
    lstm_d   = to,   
...
lstm_afternoon_std <- lstm_afternoon_std %>%
...
lstm_afternoon_canonical <- lstm_afternoon_std %>%
  transmute(
    model,
    day_abs, day_order,
    period,                      # "morning"
...
readr::write_csv(lstm_afternoon_canonical, "lstm_afternoon_canonical_period.csv")
