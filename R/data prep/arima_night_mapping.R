...
arima_night <- readr::read_csv("arima_night.csv")

# 統一命名
arima_night_std <- arima_night %>%
  ...
  mutate(model = "ARIMA", period = "night")
...
# 對齊「天數」
arima_night_std <- arima_night_std %>%
...
arima_night_canonical <- arima_night_std %>%
  select(
...
readr::write_csv(arima_night_canonical, "arima_night_canonical.csv")
